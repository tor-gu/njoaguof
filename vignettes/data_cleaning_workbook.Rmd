...
This field is not currently strictly numeric -- it also contains values `"Unknown"` and `"Under 18"`. We will add a boolean column, `juvenile` and convert `age` to an integer. The value of `"Unknown"` will map to `NA` in both columns.

The source has used more than one wording for the under-age value: `"Juvenile"` in early
releases and `"Under 18"` since. Match both.
...
We did not capture the `"Not injured"` or empty-string value in the `incident_officer_injury_type` table, but that value is largely redundant with `OfficerInjuriesInjured` which is true precisely when `OfficerInjuryType`is `"Not Injured"` or the empty string, except in 8 rows, which are evidently miscoded. (Compare case-insensitively: this field arrived as `"True"`/`"False"` through the 2024-10-31 release and as `"true"`/`"false"` from 2026-08-31 on.)
